...
wandb.init(project="contrastive_probe")

os.environ["TOKENIZERS_PARALLELISM"] = "false"

from utils.data_loader import (
    ContrastiveTuningDataset,
    DictionaryDataset,
    QueryDataset,
    QueryDataset_pretraining,
)
from utils.metric_learning import Sap_Metric_Learning, Sap_Metric_Learning_pairwise
...
def parse_args():
    ...
    parser.add_argument("--mask_percent", default=0.5, type=float)
    ...
    # help="number of chars to be randomly erased on one side of the input")
...
def main(args):
    ...
    if args.dual_encoder:
        model_wrapper = Model_Wrapper_dual_encoder()
        encoder_query, encoder_answer, tokenizer = model_wrapper.load_bert(
            path=args.model_dir,
    ...
    mask_token_id = tokenizer.encode(tokenizer.mask_token)[1]
    print("[MASK] token ID:", mask_token_id)

    # turn off all dropouts
    encoder.embeddings.dropout = torch.nn.Dropout(
        p=args.dropout_rate
    )  # args.change_dropout)
    for i in range(len(encoder.encoder.layer)):
        encoder.encoder.layer[i].attention.self.dropout = torch.nn.Dropout(
            p=args.dropout_rate
        )
        encoder.encoder.layer[i].attention.output.dropout = torch.nn.Dropout(
            p=args.dropout_rate
        )
        encoder.encoder.layer[i].output.dropout = torch.nn.Dropout(p=args.dropout_rate)

    # BERT drophead
    # https://raw.githubusercontent.com/Kirill-Kravtsov/drophead-pytorch/master/drophead.py
    # if args.drophead_rate != 0:
    #    set_drophead(encoder, args.drophead_rate)

    # load SAP model
    if args.dual_encoder:
        model = Sap_Metric_Learning_dual_encoder(
            encoder_query=encoder_query,
            encoder_answer=encoder_answer,
    ...
    def collate_fn_batch_encoding_pairwise(batch):
        ...
        if args.random_erase != 0:
            erase_len = args.random_erase # 3 tokens
            for i in range(len(query_encodings2["input_ids"])):
                # find the index of last non-zero element
                max_nonzero_index = torch.max(torch.nonzero(query_encodings2["input_ids"][i]))
                erase_len = int((max_nonzero_index-1.0) * args.random_erase) # rm 1/k tokens
                #erase_len = 1
                if erase_len == 0: continue
                if max_nonzero_index-erase_len <= 1: continue
                #if max_nonzero_index-erase_len <= 3: continue # if too short, then skip
                rand_index = torch.randint(1,max_nonzero_index-erase_len, ())
                for ii in range(erase_len):
                    query_encodings2["input_ids"][i][rand_index+ii] = mask_token_id
        #print (query_encodings1)
        #print (query_encodings2)
        #exit()
        """
    ...
    def collate_fn_batch_encoding(batch):
        ...
        query_ids = torch.tensor(list(query_id))
        return query_encodings, query_ids

    train_set = ContrastiveTuningDataset(
        path=args.train_dir, tokenizer=tokenizer, mask_percent=args.mask_percent
    )
